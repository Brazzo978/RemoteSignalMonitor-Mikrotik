...
<html lang="it">
  ...
  <body class="pb-5">
          ...
          <div class="tab-pane active" data-tab="connessione">
            ...
            <form id="connection-form" class="row g-3">
              ...
              <div class="col-12">
                ...
                <small class="text-muted">Attiva Quectel per usare AT+QCAINFO e parser dedicato.</small>
              </div>
          ...
          <div class="tab-pane" data-tab="segnali">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
              <div>
                ...
                <small class="text-muted">Dati raccolti via AT (FOX: ATI/AT^DEBUG?/AT^TEMP? • Quectel: ATI/AT+QCAINFO/AT+QTEMP)</small>
              </div>
                ...
                <div class="card border-0 shadow-sm h-100">
                  <div class="card-body">
                    ...
                    <div class="d-flex justify-content-end gap-2 mt-3">
                      <button class="btn btn-outline-secondary btn-sm" type="button" id="lock-lte-reboot">Riavvia modem</button>
                  ...
                  <div class="card-body">
                    ...
                    <div class="row g-2 mt-3">
                      <div class="col-md-6">
                        ...
                        <input class="form-control" id="lock-nr-band" placeholder="ES: 78" />
                      </div>
                      <div class="col-md-6">
                        <label class="form-label small fw-semibold">SCS</label>
                        <input class="form-control" id="lock-nr-scs" placeholder="ES: 1" />
                      </div>
                      <div class="col-md-6">
                        <label class="form-label small fw-semibold">NR-ARFCN</label>
                        <input class="form-control" id="lock-nr-arfcn" placeholder="ES: 627264" />
                      </div>
                      <div class="col-md-6">
                        <label class="form-label small fw-semibold">PCI</label>
                        <input class="form-control" id="lock-nr-pci" placeholder="ES: 148" />
                      </div>
                    </div>
                    <div class="d-flex justify-content-end gap-2 mt-3">
                      <button class="btn btn-outline-secondary btn-sm" type="button" id="lock-nr-reboot">Riavvia modem</button>
    ...
    <script>
      ...
      const lockLteRebootButton = document.getElementById('lock-lte-reboot');
      const lockNrApplyButton = document.getElementById('lock-nr-apply');
      const lockNrRebootButton = document.getElementById('lock-nr-reboot');
      ...
      lockLteRebootButton.addEventListener('click', rebootModem);
      lockNrApplyButton.addEventListener('click', applyNrLock);
      lockNrRebootButton.addEventListener('click', rebootModem);
      ...
      function toggleLockControls(locked, connected = true) {
        ...
        lockLteRebootButton.disabled = !connected;
        lockNrApplyButton.disabled = disabled;
        lockNrRebootButton.disabled = !connected;
      ...
      function addLteLockRow(pci = '', earfcn = '') {
        ...
        row.innerHTML = `
          <div class="flex-grow-1">
            <label class="form-label small fw-semibold">PCI</label>
            <input class="form-control lock-lte-pci" placeholder="ES: 405" value="${pci}">
          </div>
          <div class="flex-grow-1">
            <label class="form-label small fw-semibold">EARFCN</label>
            <input class="form-control lock-lte-earfcn" placeholder="ES: 40936" value="${earfcn}">
          </div>
          <div>
      ...
      function renderLteLockPairs(pairs, locked) {
        lockLteContainer.innerHTML = '';
        const suggested = !locked ? getSuggestedLtePairs() : [];
        if (!pairs.length && suggested.length) {
          suggested.forEach(pair => addLteLockRow(pair.pci, pair.earfcn));
      ...
      function getSuggestedLtePairs() {
        if (!lastSignals) return [];
        const pci = String(lastSignals.pci || '').trim();
        const channel = String(lastSignals.channel || '').trim();
        if (!/^\\d+$/.test(pci) || !/^\\d+$/.test(channel)) {
          return [];
        }
        return [{ pci, earfcn: channel }];
      }
      ...
      function applyLockState(state) {
        ...
        toggleLockControls(state.locked, true);
        if (state.locked) {
      ...
      async function rebootModem() {
        if (!sessionToken) {
          setLockStatus('Connetti il modem per riavviare.', 'warning');
          return;
        }
        setLockStatus('Riavvio modem in corso...', 'info');
        try {
          const response = await fetch('/send', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ token: sessionToken, command: 'AT+RESET' })
          });
          const data = await response.json();
          if (!response.ok) {
            throw new Error(data.error || 'Riavvio fallito');
          }
          setLockStatus('Comando di riavvio inviato al modem.', 'success');
        } catch (error) {
          setLockStatus('Errore riavvio modem: ' + error.message, 'error');
        }
      }
